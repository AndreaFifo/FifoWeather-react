...
export const fetchData = async (options) => {
    ...
    try {
        ...
        return { data: response.data, city, id: unitLang === true ? data.id : Date.now(), country: countryCode ? countryCode : coordinates.data[0].country, region: region ? region : coordinates.data[0].state };
    } catch (error) {
        console.log(error);
    }
}
